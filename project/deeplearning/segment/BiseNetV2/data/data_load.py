...
class LoadImageAndLabels(Dataset):
    def __init__(self, image_paths, label_paths, in_shape: tuple = (1024, 512), mode: str = 'train', crop_size: tuple = (640, 480)):
        ...
        self.mode = mode
        self.augment_size = (1080, 1920)
        self.VALID_IMG_EXTS = ['.jpg', '.jpeg', '.png', '.bmp']
    ...
    def load_mosaic(self, index):
        ...
        input_h, input_w = self.augment_size[0], self.augment_size[1]
    ...
    def load_mixup(self, index):
        ...
        img1 = Image.open(self.img_list[index]).convert("RGB").resize(self.augment_size, Image.BILINEAR)
        mask1 = Image.open(self.label_list[index]).resize(self.augment_size, Image.NEAREST)

        img2 = Image.open(self.img_list[index2]).convert("RGB").resize(self.augment_size, Image.BILINEAR)
        mask2 = Image.open(self.label_list[index2]).resize(self.augment_size, Image.NEAREST)
