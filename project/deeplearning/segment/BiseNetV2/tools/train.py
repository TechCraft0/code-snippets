...
def log_memory_usage(stage=""):
    ...
    try:
        ...
        logging.info(f"🧠 [MEMORY] {stage} | RAM: {mem.percent:.1f}% ({mem.used/1024**3:.1f}GB/{mem.total/1024**3:.1f}GB) | "
    ...
    while iter_count < max_iter:
        for images, labels in train_loader:
            ...
            if iter_count % 50 == 0:
                ...
                train_log_msg = f"📊 [{bar}] {percentage:5.1f}% Iter {iter_count:>6} | Total: {loss.item():.4f} | {loss_details} | LR: {current_lr:.2e}"
                # print(train_log_msg)
                logging.info(train_log_msg)
            ...
            if (
                ...
                try:
                    ...
                    # 验证结果输出和日志
                    val_log_msg = f"🔍 Validation | Iter {iter_count} | Loss: {val_loss:.4f} | mIoU: {miou:.4f} | PixelAcc: {pixel_acc:.4f}"
                    # print(val_log_msg)
                    logging.info(val_log_msg)
            ...
            if (
                ...
                try:
                    ...
                    # 测试结果输出和日志
                    test_log_msg = f"🏆 Test | Iter {iter_count} | Loss: {test_loss:.4f} | mIoU: {test_miou:.4f} | PixelAcc: {test_pixel_acc:.4f}"
                    # print(test_log_msg)
                    logging.info(test_log_msg)
                    ...
                    if TRAIN_PARAMS.get("enable_visualization", True):
                        logging.info(f"🎨 [DEBUG] Starting visualization generation at iter {iter_count}...")
                        print(f"🎨 Starting visualization generation at iter {iter_count}...")
                        ...
                        logging.info(f"🎨 [DEBUG] Calling save_segmentation_results function with max_samples={max_samples}...")
                        saved_count = save_segmentation_results(model, test_loader, device, f"iter_{iter_count}", class_colors, visualization_save_path, max_samples=max_samples)
                        logging.info(f"🎨 [DEBUG] Visualization saved {saved_count} samples")
                        
                        logging.info(f"🎨 [DEBUG] Visualization function completed")
                        print(f"🎨 Test visualizations saved for iter {iter_count} ({saved_count} samples)")
                        logging.info(f"🎨 Test visualizations saved for iter {iter_count} ({saved_count} samples)")
                    else:
                        logging.info(f"🎨 [DEBUG] Visualization disabled, skipping...")
                        print(f"🎨 Visualization disabled, skipping...")
                ...
                logging.info(f"🏆 [DEBUG] Setting model back to train mode")
                model.train()
...
def main():
    ...
    if enable_resume:
        ...
        if os.path.exists(root_dir):
            exp_dirs = [d for d in os.listdir(root_dir) if d.startswith(PATHS["name"] + "_") and os.path.isdir(os.path.join(root_dir, d))]
            if exp_dirs:
    ...
    if enable_resume and os.path.exists(model_save_path) and os.listdir(model_save_path):
        checkpoint_files = [f for f in os.listdir(model_save_path) if f.startswith("model_iter_") and f.endswith(".pth")]
        if checkpoint_files:
